...
class SpWsModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.visual_names = ['realInput_A','realInput_B','realInput_C','realInput_D','realInput_E','realInput_F','realOutput_A','realOutput_B','realOutput_C','realOutput_E','fake_B_1','fake_B_2','fake_B_3','fake_C'];
       # specify the models you want to save to the disk. The training/test scripts will call <BaseModel.save_networks> and <BaseModel.load_networks>
        ...
        self.netG = networks.define_G(opt.input_nc*2, opt.output_nc*3, opt.ngf, opt.netG, 0,opt.norm,
                                      not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids);
        self.netG_2 = networks.define_G(opt.input_nc*6, opt.output_nc*1, opt.ngf, opt.netG,0, opt.norm,
                                      not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids)

        if self.isTrain:  # define a discriminator; conditional GANs need to take both input and output images; Therefore, #channels for D is input_nc + output_nc
            self.netD = networks.define_D(opt.input_nc*2 + opt.output_nc*3, opt.ndf, opt.netD,
                                          opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain, self.gpu_ids);
            self.netD_2 = networks.define_D(opt.input_nc*6 + opt.output_nc*1, opt.ndf, opt.netD,
                                          opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain, self.gpu_ids)
        if self.isTrain:
    ...
    def set_input(self, input):
        ...
        self.realOutput_B = input['H'].to(self.device)
        self.realOutput_C = input['I'].to(self.device)
        self.realOutput_E = input['J'].to(self.device)
    ...
    def forward(self):
        ...
        # Forward pass stage-II
        self.fake_C=self.netG_2(self.real_C);

    def backward_D(self):
        # Stage-I GAN loss for discriminator
        ...
        # Stage-II GAN loss for discriminator
    ...
    def backward_G(self):
        # Calculate GAN and L1 loss for the generator stage-I
        ...
        # Calculate GAN and L1 loss for the generator stage-II
        # First, G(A) should fake the discriminator
